...
from bot import load_session_map, save_session_map
...
class TestSessionPersistence:
    ...
    def test_load_nonexistent_file(self, monkeypatch):
        import bot
        original = bot.SESSION_DB
        from pathlib import Path
        bot.SESSION_DB = Path("/nonexistent/test_sessions.json")
        try:
            result = load_session_map()
            assert result == {}
        finally:
            bot.SESSION_DB = original
